...
class MinesweeperTest < Gosu::Window
    def initialize
        ...
        @TILE_COLORS = {
            ...
            default: Gosu::Color.new(64,64,64),
    ...
    def draw
        ...
        @BIG_FONT.draw_text("FAILURE! (f5)", 40, (@GRID_HEIGHT * (@TILES_SIZE + @TILES_SPACING))/2 - 30 + @TOP_OFFSET, 0, 1, 1, Gosu::Color.new(255, 0, 0)) if @game_status == :lost
        @BIG_FONT.draw_text("SUCCESS! (f5)", 40, (@GRID_HEIGHT * (@TILES_SIZE + @TILES_SPACING))/2 - 30 + @TOP_OFFSET, 0, 1, 1, Gosu::Color.new(0, 255, 0)) if @game_status == :won
    end
    def needs_cursor?
        true
    end
